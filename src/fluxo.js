...
// Horário de funcionamento: MESMA regra que o painel usa (public/horario.js, dual-mode).
...
function estaAberto(tenantDir) {
  return horario.abertoAgora(store.getConfig(tenantDir));
}

// Próxima abertura a partir de agora (fuso BR), em texto curto p/ a variável
// {proximaAbertura} na mensagem de "fechado".
function proximaAbertura(config) {
  return horario.proximaAbertura(config);
}

// Texto do horário de funcionamento p/ a variável {horario} na mensagem de
// "fechado". Garante que o cliente sempre veja o horário atualizado; sem nenhum
// dia aberto na tabela, cai no texto legado que o tenant tinha salvo.
function textoHorario(config) {
  return horario.textoHorario(config.horarios) || config.restaurante.horario || "";
}
