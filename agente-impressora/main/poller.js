...
// idem para a fila generica (ids ja impressos, confirmacao pendente).
const filaImpressaLocais = new Set();
...
// Mesmo padrão de idempotência do delivery (set local p/ não reimprimir se só a
// confirmação falhar). Best-effort: erros não derrubam o ciclo principal.
async function processarFila(opts) {
  ...
  for (const item of (itens || [])) {
    const id = item.id;
    if (filaImpressaLocais.has(id)) {
      try { await api.post("/api/agente/fila/" + id + "/impresso", {}); filaImpressaLocais.delete(id); }
      catch (_) { /* tenta de novo no proximo ciclo */ }
      continue;
    }
    try {
      const buffers = montarJobDeVias(item.vias, cfg);
      if (buffers.length) await transporte.enviar(buffers, cfg);
      filaImpressaLocais.add(id);
    } catch (e) {
      log("Falha ao imprimir trabalho #" + id + ": " + e.message + " (retenta).");
      continue;
    }
    try { await api.post("/api/agente/fila/" + id + "/impresso", {}); filaImpressaLocais.delete(id); log("Impressao #" + id + " (" + (item.tipo || "") + ") concluida."); }
    catch (e) { log("Impressao #" + id + " feita, mas falhou ao confirmar no servidor (remarca depois)."); }
  }
}
...
async function umCiclo(opts) {
  ...
  // Processa a fila genérica (PDV/Mesas/Caixa/reimpressão) no mesmo ciclo.
  await processarFila(opts);
  return INTERVALO_MS;
}
