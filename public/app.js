...
function renderCategorias() {
  ...
  cont.className = "cat-cards";
  cont.innerHTML = "";
  cats.forEach((cat, i) => {
    const n = (cat.itens || []).length;
    const ativo = cat.ativo !== false;
    const card = document.createElement("div");
    card.className = "cat-card" + (ativo ? "" : " inativa");
    const icoOlho = ativo
    ...
    card.innerHTML =
      '<div class="cat-card-linha1">' +
        '<span class="cat-card-nome" data-i="' + i + '">' + escapar(cat.nome || "") + '</span>' +
        '<div class="cat-card-acoes">' +
          '<button type="button" class="cat-ico cat-edit" data-i="' + i + '" aria-label="Editar nome" title="Editar nome"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4z"/></svg></button>' +
          '<button type="button" class="cat-ico cat-ativo" data-i="' + i + '" aria-label="' + (ativo ? "Desativar (esconder do cardápio)" : "Ativar") + '" title="' + (ativo ? "Desativar" : "Ativar") + '">' + icoOlho + '</button>' +
          '<button type="button" class="cat-ico cat-del" data-i="' + i + '" aria-label="Excluir categoria" title="Excluir"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>' +
        '</div>' +
      '</div>' +
      '<div class="cat-card-count">' + n + (n === 1 ? " item" : " itens") + (ativo ? "" : " · inativa") + '</div>';
    cont.appendChild(card);
  });
  // Ativar/desativar (ícone de olho) — desativar esconde a categoria (e seus itens) do cardápio e do PDV.
  cont.querySelectorAll(".cat-ativo").forEach((b) => b.addEventListener("click", async () => {
    const idx = +b.dataset.i, cat = cardapioAtual.categorias[idx], n = (cat.itens || []).length;
    const antes = cat.ativo;
    const novaAtiva = (cat.ativo === false);   // estava inativa → ativa; estava ativa → inativa
    cat.ativo = novaAtiva;
      ...
      <div class="ped-det-col">
        ...
        <div class="ped-bloco">
          ...
          <div class="ped-linha-info">
            ...
            <div class="ped-info-texto">${escapar(textoPagamento(p)) || "—"}${trocoHtml}</div>
          </div>
        </div>
