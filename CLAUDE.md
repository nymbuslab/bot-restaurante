...
public/
  ...
  grupos.js           -> PURO (dual-mode Node/browser): **biblioteca de complementos** — grupos reutilizáveis por empresa (`cardapio.grupos`, opção com id estável e preço) + vínculo por produto (`item.grupos`, regra efetiva sobrescrevível): normalizarBiblioteca/resolverGrupos/avaliarEscolhas + converterCardapio (migração). Mantém o caminho legado (normalizarGrupos/avaliarComposicao) p/ item ainda não convertido. Usado por src/cardapio-web.js e src/pdv.js — testado em test/grupos.test.js
