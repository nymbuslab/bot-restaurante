...
- **Google Stitch MCP** — protótipo de tela (configurado em `.opencode/opencode.json`, `STITCH_API_KEY`). **Sempre** que for criar tela nova, seção visual ou alterar o layout de uma tela existente, desenhar antes e mostrar ao dono, salvo quando a mudança for puramente estrutural (ex.: mover blocos de HTML existentes, renomear classes). **Semear o Stitch com os tokens REAIS de `public/style.css`** (o design system do projeto é aquele arquivo, não um cadastro na ferramenta). Projeto Stitch atual: `projects/7236747227852373120`; design system reutilizável semeado: `assets/6506830711685967852` (`Nymbus Pedidos`). Protótipos antigos em `.dc.html` (`design/canvas/`) são histórico do Claude Design, que deixou de ser usado; não precisam ser refeitos.
...
- [docs/estoque-e-custos/](docs/estoque-e-custos/README.md) — programa de evolução de Compras, Insumos, Estoque, ficha técnica e custos; contém auditoria de baseline, ordem P0/P1, gates de rollout e decisões que precisam ser fechadas antes da arquitetura nova.
- [docs/equipe.md](docs/equipe.md) — funcionários, permissões, dispositivos, PIN, Atividades e gates em homologação; produção depende de backup restaurado e rollout separado.
