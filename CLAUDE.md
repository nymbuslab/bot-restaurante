...
src/
  ...
  pagamentos.js       -> formas de pagamento PURO: vocabulário FIXO (FORMAS_PAGAMENTO: Dinheiro/PIX/Crédito/Débito) + normalizarFormasPagamento (whitelist/migra strings legadas) + formaPermitida + ehDinheiro (decide se o "Troco para" do cardápio web vale algo) (a venda valida contra a lista NORMALIZADA, que é a que a tela mostrou — comparar com a crua recusava cartão em tenant legado). `config.pagamentos` só aceita canônicos (whitelist ao salvar + normaliza na leitura do painel)
