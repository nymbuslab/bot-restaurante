...
- [docs/modelo-dados.md](docs/modelo-dados.md) — schema (`empresas` + coluna `plano`, `pedidos`, item do cardápio com **grupos de opções** `item.grupos` — substituem composição/opcionais; regra min/máx + opções grátis/cobradas, recálculo em `cardapio-web.resolverOpcoesGrupos`, `config.frete`, `geo_cache`) + **cardápio web** (API pública, recálculo no servidor, frete por raio, token de link) + estados enxutos do bot (`fluxo.js`).
