...
src/
  ...
  fiado.js            -> venda "A Prazo" (fiado): helper PURO `podeVenderAPrazo` (bloqueio por limite/vencimento, liberação pontual) + `venderAPrazo` (PDV Balcão) / `fecharMesaAPrazo` (Mesa) que criam pedido `a_prazo` vinculado a cliente SEM movimento no caixa (**vencimento calculado pelo Convênio do cliente**, `public/convenios.js`) + `resumoDoCliente` (gasto/saldo/vencido das vendas a prazo em aberto). **Contas a Receber (Fase 4):** `listarContas` (clientes com fiado aberto/quitado agregado + resumo p/ cards: a receber/em atraso/novos no mês), `vendasDoCliente` (vendas do cliente + log de baixas p/ o modal) e `baixar` (recebimento integral OU parcial, em lote; entra no caixa do dia SÓ no Completo — `comCaixa` do servidor — senão só quita; grava log em `fiado_baixas` e marca `recebido_em` quando quita). Testado em test/fiado.test.js + smoke
