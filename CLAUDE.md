...
public/
  ...
  estoque.js          -> PURO (dual-mode Node/browser): cálculo do estoque — baixa e devolução da venda, diff entre dois cardápios (vira movimento de `ajuste`), ajuste manual (entrada SOMA, perda SUBTRAI, contagem SUBSTITUI o saldo), mínimo, formatarQtd (un inteiro, kg com vírgula), linhasDeEstoque (uma linha por saldo, para a tela) preservarSaldos (salvar o cardápio NÃO desfaz venda concorrente: só o item marcado com `_estoqueEditado` traz saldo próprio, o resto volta ao do banco) e saldoMudou (**quem decide o carimbo**: o editor só marca quando `estoque`/`estoqueMinimo` do produto ou de alguma variação mudou de valor — carimbar por abrir o produto fazia uma edição de preço reescrever o saldo com o número do boot; a marca também é apagada assim que o salvamento dá certo, senão contaminava todo salvamento seguinte do cardápio inteiro) — testado em test/estoque.test.js
