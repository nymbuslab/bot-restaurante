...
src/
  ...
  impressao-fila.js   -> fila de impressão GENÉRICA (tabela impressao_fila): o servidor renderiza o TEXTO das vias e enfileira (PDV/Mesas/Caixa/reimpressão); comprovantes avulsos do caixa (`caixa-comprovante`) entram aqui conforme `config.impressao.caixa`; o agente busca, imprime e marca. Delivery NÃO usa (segue pelo polling de `pedidos`)
...
public/
  ...
  comprovante-caixa.js -> PURO (dual-mode Node/browser): monta comprovantes 80mm de suprimento, sangria e cancelamento pago, enfileirados pelo servidor conforme `config.impressao.caixa` — testado em test/comprovante-caixa.test.js
