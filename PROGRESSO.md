...
- [x] **Backup pelo painel super-admin** — aba **Configurações** no `/admin-master` (estrutura pronta p/ novas subseções) com subseção **Backup**: gerar (estado "Gerando…"), listar (arquivo/tamanho/data) e **baixar** pro PC (blob autenticado). Seção "Como restaurar" (somente leitura) renderizada do `DEPLOY.md` (fonte única via marcadores `RESTAURACAO:START/END`, renderizador de markdown que escapa HTML, fallback se o arquivo sumir); deixa explícito que a restauração é manual e que o painel não executa. Rotas sob `exigeSuperAdmin` reusam `scripts/backup.js` (refatorado p/ expor `gerarBackup`/`listarBackups`; `npm run backup` intacto); download com **validação anti-path-traversal**. Validado por Playwright (8 cenários: trocar aba, gerar→lista atualiza, baixar, runbook renderizado, mobile) + curl (anti-traversal: 4 nomes maliciosos → 400, inexistente → 404, tudo 401 sem token)
