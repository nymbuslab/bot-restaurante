...
- Validado: adapter de sessão (round-trip de serialização com Buffers, 8 checks) + bot real chegando ao **QR** (prova o adapter + `makeWASocket`) + upload→Storage→URL pública acessível + JWT local (warm ~144 ms, token forjado → 401) + exclusão limpando banco/sessão/Storage. **Ressalva:** a prova final das sessões (escanear o QR → reiniciar → reconectar sem novo QR) exige um celular real
