...
- **Confirmação destrutiva identifica o alvo:** ao excluir ou remover um registro, o título ou a mensagem do modal mostra o nome do item, categoria, grupo, mesa ou outro registro. Nunca pedir confirmação genérica quando o usuário escolheu uma linha de uma lista.
...
- **Foco de modal (automático):** `public/a11y.js` cuida do foco dos overlays. Ao **abrir** um modal, o foco entra sozinho no 1º campo focável visível; o Tab fica preso dentro (focus-trap). **Não** escrever `.focus()` na mão ao abrir modal, nem tratar o Tab — é retrabalho e conflita com o util. Só é preciso garantir que o overlay case com a lista `SEL` do `a11y.js` (as classes padrão `.modal-overlay`/`.pdv-modal`/`.cd-modal`/`.cd-sheet`/`.mesa-painel` já casam). Cuidado ao usar o **primeiro** foco-possível na mão: um botão de fechar `display:none` (os "X" removidos) não recebe foco.
